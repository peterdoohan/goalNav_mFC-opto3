...
class Rsync_aligner():
    ...
    def __init__(self, pulse_times_A, pulse_times_B, units_A='auto', units_B='auto', 
        ...
        # Evalute inter pulse intervals in units of ms.
        ...
        # Compute variables needed for extrapolating beyond first/last matching pulse.
        diff_cor_times_B = np.diff(cor_times_B)
        self.dAdB = (np.sum(np.diff(pulse_times_A)[~np.isnan(diff_cor_times_B)])/ # Empirical units_A/units_B from matched intervals.
    ...
    def A_to_B(self, times_A, extrapolate=False):
    ...
    def B_to_A(self, times_B, extrapolate=False):
...
def check_extrapolation():
    pulse_times_A, pulse_times_B = simulate_pulses(n_pulse=100)
    aligner = Rsync_aligner(pulse_times_A, pulse_times_B, plot=False)
    times_A = np.arange(-20000,130000, 5000)
    times_B = 2*np.arange(-22500,132500, 5000)
    plt.figure(2, clear=True)
    plt.plot(pulse_times_A, pulse_times_B, '.')
    plt.plot(times_A, aligner.A_to_B(times_A, extrapolate=True), '.')
    plt.plot(aligner.B_to_A(times_B, extrapolate=True), times_B, '.')
